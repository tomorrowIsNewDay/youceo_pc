...
var drag = {
	...
	init : function (className){
		...
		$('.' + drag.class_name).on('mousedown','dd', function(event){
	...
	mousemove : function (event){
		...
		//判断当前容器是否允许放置节点
		$.each($('.jobAnalysisCenter-text'), function(index, value){
	...
	mouseup : function (event){
		...
		$.each($('dl'), function(index, value){
			...
			if(e.pageX > box_x && e.pageX < box_x-0+box_width && e.pageY > box_y && e.pageY < box_y-0+box_height){
				...
				|| $(value).offset().top !== drag.old_elm.parent().offset().top){
					//向目标容器添加节点并删除原节点
					tmp = $(drag.old_elm).children('i').prop("outerHTML")
					console.log(tmp)
					var tmp_text = $(drag.old_elm).text();
					var newdd = '<dd class="docBox"><i class="icon iconfont icon-download pull-left"> </i>'+tmp+
						'<i class="icon iconfont icon-close pull-right"></i>' +
						'<p>'+tmp_text+'</p>'+	
					'</dd>';
					var newObj = $(value).prepend(newdd);
					var num =$(value).prev('h3').find('.docBoxNum').text();
					num++;
					$(value).prev('h3').find('.docBoxNum').text(num);
					$(drag.old_elm).remove();
					//获取新添加节点的对象
