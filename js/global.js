...
//下拉框
function dorpdown(e){
	var _img = e.children('img').clone();//找到 img
	var $span = e.parents('.dropdownMenu').prev('span');
	$span.addClass('isSpan');//添加 类名
	$span.html(_img);
	var del = '<i class="icon iconfont icon-close delIcon" ></i>';//关闭 图标
	$span.append(del);
	e.parents('.dropdownMenu').slideUp();//关闭 下拉框
}
// // 关闭 图标
